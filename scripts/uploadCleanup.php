 ...
 *
 * - Find and remove files and folders that don't look like uploads or thumbnails.
 *
...
function removeWithMessage($path, $message) {
  global $dryRun;
  ...
  foreach ($files as $file) {
    if (!Str::startsWith($file, '.')) {
      ...
      try {
        ...
        removeWithMessage($full, $e->getMessage());
      }

      // remove legitimate directories if they are left empty
      if (isEmptyDir($full)) {
        removeWithMessage($full, 'Directory is empty');
      }
    }
  }
}
